(function () {
  ...
  const CHAVE_DISPLAYS_VISIVEIS = "p3.displays7seg.visivel";
  ...
  function definirVisibilidadeDisplays(visivel) {
    ...
    try {
      window.localStorage.setItem(CHAVE_DISPLAYS_VISIVEIS, visivel ? "1" : "0");
    } catch (erro) {
      // ignora falhas de persistencia local
    }
  }

  function obterVisibilidadeInicialDisplays() {
    try {
      const guardado = window.localStorage.getItem(CHAVE_DISPLAYS_VISIVEIS);
      return guardado === null ? true : guardado === "1";
    } catch (erro) {
      return true;
    }
  ...
  function iniciarAplicacao() {
    ...
    definirVisibilidadeDisplays(obterVisibilidadeInicialDisplays());
