(function () {
  ...
  function atualizarPrograma(cpu) {
    ...
    caixa.innerHTML = linhas.map(linha => `
      <div class="linha-programa${linha.ativa ? " ativa" : ""}${linha.breakpoint ? " com-breakpoint" : ""}">
        ${linha.clicavel ? `
          <button
            type="button"
            class="breakpoint-programa${linha.breakpoint ? " ativo" : ""}"
            data-address="${linha.address}"
            aria-label="${linha.breakpoint ? "Remover" : "Adicionar"} ponto de quebra em ${formatarPalavra(linha.address)}"
            title="${linha.breakpoint ? "Remover" : "Adicionar"} ponto de quebra em ${formatarPalavra(linha.address)}"
          ></button>
    ...
    caixa.querySelectorAll(".breakpoint-programa").forEach(botao => {
      botao.addEventListener("click", () => {
        ...
        if (state.breakpoints.has(endereco)) {
          ...
          window.P3UI?.mostrarErros?.([`Ponto de quebra adicionado em ${formatarPalavra(endereco)}.`]);
        }
